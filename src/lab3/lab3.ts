...
export function csvToJSON(input: string[], delimiter: string): object[] {
	...
	if (input.length === 0) {
		throw new Error(
			'Ошибка: массив input должен содержать хотя бы строку заголовка.',
		);
	}
	...
	if (headers.length === 0 || headers.some((h) => h.trim().length === 0)) {
		throw new Error(
			'Ошибка: некорректный заголовок CSV (есть пустые названия столбцов).',
		);
	}
	...
	for (let rowIndex = 1; rowIndex < input.length; rowIndex++) {
		...
		for (let colIndex = 0; colIndex < headers.length; colIndex++) {
			...
			const value: string | number =
				trimmed !== '' && Number.isFinite(num) && String(num) === trimmed
					? num
					: raw;
...
export async function formatCSVFileToJSONFile(
	...
	if (typeof input !== 'string' || input.length === 0) {
		throw new Error(
			'Ошибка: путь к входному CSV-файлу должен быть непустой строкой.',
		);
	}
	if (typeof output !== 'string' || output.length === 0) {
		throw new Error(
			'Ошибка: путь к выходному JSON-файлу должен быть непустой строкой.',
		);
	}
	if (typeof delimiter !== 'string' || delimiter.length === 0) {
		throw new Error('Ошибка: разделитель (delimiter) должен быть непустой строкой.');
