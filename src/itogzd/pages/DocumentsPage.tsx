...
import {
	...
	exportCellsToJson,
	parseCsv,
} from '../features/documents/exportImport';
...
export function DocumentsPage() {
	...
	function importCsvToDocument(documentId: string, file: File, title?: string) {
		...
		reader.onload = () => {
			...
			const cells: Record<
				string,
				{ raw: string; computed: string; type: 'string' }
			> = {};
	...
	function deleteDocument(documentId: string) {
		...
		const confirmed = window.confirm(
			'Удалить документ? Это действие нельзя отменить.',
		);
		...
		localStorage.setItem(documentsListKey, JSON.stringify(updatedLocalDocuments));
	}
					...
					<button
						...
						onClick={() => {
							...
							input.onchange = (changeEvent: Event) => {
								const file = (changeEvent.target as HTMLInputElement)
									.files?.[0];
