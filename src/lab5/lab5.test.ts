...
describe('expectTypeOf: неверный порядок', () => {
	it('sort перед where — ошибка', () => {
		const q = query<User>(sort<User>()('age'), where<User>()('name', 'John'));
		expectTypeOf(q).toBeFunction();
	});

	it('where после groupBy — ошибка', () => {
		const q = query<User>(groupBy<User>()('city'), where<User>()('name', 'John'));
		expectTypeOf(q).toBeFunction();
	});
	...
	it('sort перед groupBy — ошибка', () => {
		const q = query<User>(sort<User>()('age'), groupBy<User>()('city'));
		expectTypeOf(q).toBeFunction();
	});
});
