...
import type { AirPollutionResponse, ForecastListItem, GeocodeResult, FiveDayForecastResponse } from "./types"
import {
  getAqiLabel,
...
function buildDaySummaries(forecast: FiveDayForecastResponse | null | undefined): DaySummary[] {
...
function WeatherIcon({ iconCode, alt, size = 44 }: { iconCode: string; alt: string; size?: number }) {
  return (
    <img
      ...
      width={size}
      height={size}
      style={{ display: "block", filter: "drop-shadow(0 4px 10px rgba(0,0,0,0.25))" }}
    />
  )
}
...
function CitySelector({
  ...
  return (
    <section style={{ width: "100%", maxWidth: 420, margin: "0 auto 18px" }}>
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 12,
            border: "none",
            background: loading ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.18)",
            color: "#fff",
      ...
      {error && <p style={{ margin: "10px 0 0", color: "rgba(255,255,255,0.9)" }}>{error}</p>}
    </section>
  )
}
    ...
    <section style={{ width: "100%", maxWidth: 420, margin: "0 auto" }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 18, color: theme.text }}>{cityLabel}</div>
            <div style={{ marginTop: 6, color: theme.mutedText }}>
              {nowDate ? `${formatWeekdayRu(nowDate)}, ${nowDate.getDate()}.${nowDate.getMonth() + 1}` : ""}
            </div>
          </div>
          {nowItem && (
            <div style={{ textAlign: "right" }}>
              <div style={{ color: theme.text, fontWeight: 900, fontSize: 44, lineHeight: 1 }}>
                {Math.round(nowItem.main.temp)}°
              </div>
              <div style={{ marginTop: 8 }}>
                <WeatherIcon iconCode={nowItem.weather[0]?.icon ?? "01d"} alt="Сейчас" size={52} />
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ marginTop: 14, display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ color: theme.mutedText, fontSize: 14, fontWeight: 700 }}>На ближайшие часы</div>
            <div style={{ display: "flex", gap: 12, marginTop: 10 }}>
              {hourlyItems.map((it) => (
                <div key={it.dt} style={{ width: 72, textAlign: "center" }}>
                  <div style={{ color: theme.mutedText, fontSize: 13, fontWeight: 700 }}>{formatTimeHHMM(it.dt)}</div>
                  ...
                  <div style={{ color: theme.text, fontWeight: 900, marginTop: 4 }}>{Math.round(it.main.temp)}°</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {nowItem && (
          <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <Metric label="Влажность" value={`${nowItem.main.humidity}%`} theme={theme} />
            <Metric label="Ветер" value={`${Math.round(nowItem.wind.speed)} м/с`} theme={theme} />
            <Metric label="Давление" value={`${pressureHpaToMmHg(nowItem.main.pressure)} мм рт.ст.`} theme={theme} />
            <Metric label="Облачность" value={`${nowItem.clouds.all}%`} theme={theme} />
          </div>
        )}

        <div style={{ marginTop: 16, borderTop: "1px solid rgba(255,255,255,0.18)", paddingTop: 14 }}>
          <div style={{ color: theme.text, fontWeight: 900, marginBottom: 12 }}>Прогноз на дни</div>
            ...
            {daySummaries.map((d) => (
              ...
              >
                <div style={{ color: theme.text, fontWeight: 900 }}>
                  {formatWeekdayRu(d.date)}, {d.date.getDate()}.{d.date.getMonth() + 1}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <WeatherIcon iconCode={d.icon} alt="День" size={34} />
                  <div style={{ textAlign: "right" }}>
                    <div style={{ color: theme.mutedText, fontSize: 13, fontWeight: 700 }}>
                      min {Math.round(d.minTemp)}°
                    </div>
                    <div style={{ color: theme.text, fontWeight: 900 }}>{Math.round(d.maxTemp)}°</div>
                  </div>
                </div>
              </div>
            ))}
...
function Metric({ label, value, theme }: { label: string; value: string; theme: Theme }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 14, padding: "10px 12px" }}>
      <div style={{ color: theme.mutedText, fontWeight: 700, fontSize: 13 }}>{label}</div>
      <div style={{ color: theme.text, fontWeight: 900, marginTop: 6 }}>{value}</div>
    </div>
  )
}
    ...
    <section style={{ width: "100%", maxWidth: 420, margin: "18px auto 0" }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <div style={{ color: theme.text, fontWeight: 900, fontSize: 18 }}>Загрязнение воздуха</div>
            <div style={{ marginTop: 6, color: theme.mutedText, fontWeight: 700 }}>
              AQI: {aqi} - {aqiUi.label}
            </div>
          </div>
          <div style={{ color: theme.accent, fontWeight: 900, fontSize: 46, lineHeight: 1 }}>
            {aqi}
          </div>
        </div>
        ...
        <div style={{ marginTop: 14 }}>
          {shown.length === 0 ? (
            <div style={{ color: theme.mutedText, fontWeight: 700 }}>Поля загрязнения недоступны</div>
          ) : (
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {shown.map((x) => (
                ...
                >
                  <div style={{ color: theme.mutedText, fontWeight: 800, fontSize: 12 }}>{x.label}</div>
                  <div style={{ color: theme.text, fontWeight: 900, marginTop: 6 }}>{x.value}</div>
                </div>
              ))}
            </div>
          )}
...
export default function Lab8_9() {
  ...
  async function handlePickCity() {
    ...
    try {
      ...
      setCoords({ coords: { lat: geo.lat, lon: geo.lon }, label: `${geo.name}${geo.country ? ", " + geo.country : ""}` })
      setForecast(null)
      setAir(null)
    } catch (e) {
    ...
    const loadAll = async () => {
      ...
      try {
        ...
        const [f, a] = await Promise.all([
          fetchForecast(apiKey, coords.coords.lat, coords.coords.lon, controller.signal),
          fetchAirPollution(apiKey, coords.coords.lat, coords.coords.lon, controller.signal),
        ])
        if (cancelled) return
        setForecast(f)
        setAir(a)
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.background, transition: "background 400ms ease" }}>
      <CitySelector
        value={cityInput}
      ...
      <div style={{ color: theme.text, textAlign: "center", padding: "0 16px" }}>
        {!coords && <p style={{ color: theme.mutedText, fontWeight: 700 }}>Выберите город</p>}
        {loading && <p style={{ color: theme.mutedText, fontWeight: 700, margin: "8px 0 0" }}>Загрузка прогноза...</p>}
        {error && <p style={{ color: "#ffb3b3", fontWeight: 800, margin: "8px 0 0" }}>{error}</p>}
        <button
      ...
      {!apiKey && (
        <section style={{ maxWidth: 520, margin: "18px auto", padding: "0 16px", color: "#fff" }}>
          <div style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: 16, padding: 14 }}>
            <div style={{ fontWeight: 900, fontSize: 18 }}>Нужен API ключ OpenWeather</div>
            <div style={{ marginTop: 8, color: "rgba(255,255,255,0.9)", fontWeight: 700 }}>
